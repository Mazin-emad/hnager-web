...
import {
  getLinesCountFormula,
  productKeys,
  updateLinesCountFormula,
} from "@/api/products";
...
/**
 * Product-level Lines Count formula editor (backend-calculated lines count).
 * Same pattern as QuantityFormulaEditor: admin prefill + variable-key chips
 * (product variables only — never the reserved `LinesCount` key) + live
 * validate + save, version display. Read-only shows the current value.
 */
export function LinesCountFormulaEditor({
  ...
  const formulaQuery = useQuery({
    queryKey: productKeys.linesCountFormula(productId),
    queryFn: () => getLinesCountFormula(productId),
  ...
  const saveMutation = useMutation({
    mutationFn: () => updateLinesCountFormula(productId, { expression: expression.trim() }),
    onSuccess: (res) => {
      setSaveError(null);
      toast.success(`تم حفظ معادلة عدد الخطوط (إصدار ${res.version})`);
      void queryClient.invalidateQueries({ queryKey: productKeys.linesCountFormula(productId) });
      ...
      <div className="space-y-2">
        {neverConfigured ? (
          <p className="text-sm text-muted-foreground">
            لا توجد معادلة لعدد الخطوط بعد.
          </p>
        ) : (
          <>
            <code
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-sm text-muted-foreground">
          يُحسب عدد الخطوط في الفاتورة من قيم المتغيرات — لا تُدخل العدد يدويًا في أي مكان.
        </p>
        {version > 0 ? (
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            لا توجد معادلة لعدد الخطوط بعد — احفظ واحدة لتفعيل الحساب.
          </p>
        )}
      </div>

      <div className="space-y-2">
        <Label htmlFor={`lines-expr-${productId}`}>معادلة عدد الخطوط</Label>
        <Textarea
          id={`lines-expr-${productId}`}
          dir="ltr"
          rows={3}
      ...
      >
        ...
        {saveMutation.isPending ? "جارٍ الحفظ…" : "حفظ معادلة عدد الخطوط"}
      </Button>
    </div>
  );
}
